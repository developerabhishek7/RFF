...
export default class CalenderComponent extends Component {
  ...
  getVisibilityArray(boundArray) {
    ...
    for (const property in boundArray) {
      if ("aeconomy" in boundArray[property] && boundArray[property]["aeconomy"].seats >= searchData.passengerCount) {
        !boundVisibleArray.includes("aeconomy") &&
          boundVisibleArray.push("aeconomy");
      }
      if ("bpremium" in boundArray[property] && boundArray[property]["bpremium"].seats >= searchData.passengerCount) {
        !boundVisibleArray.includes("bpremium") &&
          boundVisibleArray.push("bpremium");
      }
      if ("cbusiness" in boundArray[property] && boundArray[property]["cbusiness"].seats >= searchData.passengerCount) {
        !boundVisibleArray.includes("cbusiness") &&
          boundVisibleArray.push("cbusiness");
      }
      if ("dfirst" in boundArray[property] && boundArray[property]["dfirst"].seats >= searchData.passengerCount) {
        !boundVisibleArray.includes("dfirst") &&
          boundVisibleArray.push("dfirst");
      }
    }
  ...
  componentDidMount = async () => {
    ...
    console.log("yes print data on did mount - - - - - - ",classData)
  ...
  seatAvailabilityModal(day, isOffPeakValue1) {
    ...
    // console.log("yes check here available outbound   - -  - - = = =",availableOutBoundDate)
   ...
   if (requiredKey == true && this.props.isLoggedIn == false) {
      ...
      obj.map((singleMap) => {
        ...
        console.log("yes check inside the singleMap - - - -  - - -",singleMap)
       
        if (this.state.clickDate == singleMap[0]) {
          if (singleMap[1].aeconomy) {
            if (singleMap[1].aeconomy.seats) {
              economySeats = singleMap[1].aeconomy.seats
            }
          }
          else {
            economySeats = 0
          }
          if (singleMap[1].bpremium) {
            if (singleMap[1].bpremium.seats) {
              premiumSeats = singleMap[1].bpremium.seats
            }
          }
          else {
            premiumSeats = 0
          }
          if (singleMap[1].cbusiness) {
            if (singleMap[1].cbusiness.seats) {
              businessSeats = singleMap[1].cbusiness.seats
            }
          }
          else {
            businessSeats = 0
          }
          if (singleMap[1] && singleMap[1].dfirst) {
            if (singleMap[1].dfirst.seats) {
              firstSeats = singleMap[1].dfirst.seats
            }
          }
          else {
            firstSeats = 0
    ...
    console.log("yes print here economySeats    - - - - - - -",economySeats)
    console.log("yes print here premiumSeats    - - - - - - -",premiumSeats)
    console.log("yes print here businessSeats    - - - - - - -",businessSeats)
    console.log("yes print here firstSeats     - - - - - - -",firstSeats)
  ...
  ticketClass() {
    ...
    // console.log("yes check here classSelected value inside ticket class - - - - -  - -",this.state.outBoundVisibleArray)
              ...
              if(!bronzeMember){
                if (premium || business || first) {
                  this.setState({isSliderRunDone:false},()=>{
                    if (
                      (this.state.selectedIndex == 0 &&
                        this.state.outBoundVisibleArray.includes("aeconomy")) ||
                      (this.state.selectedIndex == 1 &&
                        this.state.inBoundVisibleArray.includes("aeconomy"))
                    ) {
                      ...
                      console.log("yes check heare newClassAray  -  - - - - - - -",newClassArray)
              ...
              if(!bronzeMember){
                if (economy || business || first) {
                  this.setState({isSliderRunDone:false},()=>{
                    if (
                      (this.state.selectedIndex == 0 &&
                        this.state.outBoundVisibleArray.includes("bpremium")) ||
                      (this.state.selectedIndex == 1 &&
                        this.state.inBoundVisibleArray.includes("bpremium"))
                    ) {
                      let newClassArray = this.state.classSelected;
                      let arr = this.state.classSelected
              ...
              if(!bronzeMember){
                if (economy || premium || first) {
                  this.setState({isSliderRunDone:false},()=>{
                    if (
                      (this.state.selectedIndex == 0 &&
                        this.state.outBoundVisibleArray.includes("cbusiness")) ||
                      (this.state.selectedIndex == 1 &&
                        this.state.inBoundVisibleArray.includes("cbusiness"))
                    ) {
                      let newClassArray = this.state.classSelected;
              ...
              if(!bronzeMember){
                if (economy || premium || business) {
                  this.setState({isSliderRunDone:false},()=>{
                    if (
                      (this.state.selectedIndex == 0 &&
                        this.state.outBoundVisibleArray.includes("dfirst")) ||
                      (this.state.selectedIndex == 1 &&
                        this.state.inBoundVisibleArray.includes("dfirst"))
                    ){
                      let newClassArray = this.state.classSelected;
                      let arr = this.state.classSelected
...
// ON DAY PRESS FUNCTIONALITY . . . . . . . .
  ...
  getLocations(isLoader) {
    ...
    for(let i of Object.keys(availableOutBoundDate)){ 
      if(availableOutBoundDate[i]["economy"]){
        availableOutBoundDate[i]["aeconomy"] = availableOutBoundDate[i]["economy"]
        delete availableOutBoundDate[i]['economy']
      }
        if(availableOutBoundDate[i]["premium"]){
        availableOutBoundDate[i]["bpremium"] = availableOutBoundDate[i]["premium"]
        delete availableOutBoundDate[i]['premium']
      }
      if(availableOutBoundDate[i]["business"]){
        availableOutBoundDate[i]["cbusiness"] = availableOutBoundDate[i]["business"]
        delete availableOutBoundDate[i]['business']
      }

      if(availableOutBoundDate[i]["first"]){
        availableOutBoundDate[i]["dfirst"] = availableOutBoundDate[i]["first"]
        delete availableOutBoundDate[i]['first']
      }              
    }
    ...
    for(let i of Object.keys(availableInBoundDate)){ 
          if(availableInBoundDate[i]["economy"]){
            availableInBoundDate[i]["aeconomy"] = availableInBoundDate[i]["economy"]
            delete availableInBoundDate[i]['economy']
          }
            if(availableInBoundDate[i]["premium"]){
            availableInBoundDate[i]["bpremium"] = availableInBoundDate[i]["premium"]
            delete availableInBoundDate[i]['premium']
          }
          if(availableInBoundDate[i]["business"]){
            availableInBoundDate[i]["cbusiness"] = availableInBoundDate[i]["business"]
            delete availableInBoundDate[i]['business']
          }
          if(availableInBoundDate[i]["first"]){
            availableInBoundDate[i]["dfirst"] = availableInBoundDate[i]["first"]
            delete availableInBoundDate[i]['first']
          }
    }

    let availability_data = {'aeconomy': economyClass, 'bpremium': premiumClass, 'cbusiness': businessClass, 'dfirst': firstClass }
